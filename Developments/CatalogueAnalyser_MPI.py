...
#  *WRITTEN LIBRARIES
sys.path.insert(0, '/home/anto/Desktop/test0/modules')  # modules path
...
def setup_logger(name, level=logging.INFO):
        handler = logging.FileHandler('logs/cat/'+name)  # PATH + NAME
...
# FUNCTION FOR SELECTING GAS PARTICLES (LINE:116 & 185)
@jit
...
with open('catalogue/bh_cat.p', 'rb') as fr:
        bh_cat = pl.load(fr)
with open('catalogue/snap_cat.p', 'rb') as fr:
        snap_cat = pl.load(fr)
bh_id = bh_cat[:, 0]
...
if rank == 0:
        ...
        with open('data/data.p', 'rb') as fr:
                data = pl.load(fr)
                ...
                for i in range(1, size):
                        ...
                        for bh in selection_dict.keys():
                                ...
                                LOG.info('@%d: Data updated for %s' % (
                                                          rank, bh))
                SNP.info('%s' % filename)
        with open('data/dataUpd.p', 'wb') as fw:  # SAVING THE FINAL DATASET
                pl.dump(data, fw)
